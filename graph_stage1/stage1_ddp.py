...
# 首先处理分布式参数
parser = argparse.ArgumentParser(description="图节点解释模型训练 (DDP版本)")
parser.add_argument("--local_rank", type=int, default=-1, help="本地进程排名")
temp_args, _ = parser.parse_known_args()
local_rank = temp_args.local_rank

# 初始化分布式环境
if local_rank != -1:
    print(f"初始化进程组，local_rank: {local_rank}")
    torch.cuda.set_device(local_rank)
    try:
        dist.init_process_group(backend="nccl")
        print(f"进程组初始化成功，world_size: {dist.get_world_size()}")
...
class GraphNodeModel(nn.Module):
    ...
    def forward(self, graph_embeddings, input_ids=None, attention_mask=None, labels=None):
        ...
        # 截断过长的输入序列
        max_input_length = self.max_seq_length - 1 - self.prompt_length
...
class GraphEmbeddingDataset(Dataset):
    ...
    def __init__(self, embeddings, texts, tokenizer, max_length=1024):
...
def train_graph_node_model(
    ...
    if is_main_process and use_wandb:
        ...
        wandb.login(key="fd2b686e8d1ea21dcaa11537fd6f572ccd30e303")
    ...
    # 创建数据集
    dataset = GraphEmbeddingDataset(embeddings, texts, tokenizer, max_length=max_seq_length)
    ...
    if hasattr(model, "module"):
        optimizer_params = [
            {'params': model.module.projector.parameters(), 'lr': learning_rate},
            {'params': model.module.prompt_embeddings, 'lr': learning_rate * 5.0}
        ]
    else:
        optimizer_params = [
    ...
    for epoch in range(num_epochs):
        ...
        if is_main_process:
            ...
            if avg_loss < best_loss:
                ...
                torch.save(best_model_state, "./best_model/pytorch_model.bin")
...
def main():
    ...
    # 这里不需要再次添加--local_rank，因为我们已经处理过了
    ...
    # 设置随机种子，确保可重复性
    seed = 42 + (local_rank if local_rank != -1 else 0)
    random.seed(seed)
    np.random.seed(seed)
    ...
    if is_main_process:
        try:
            ...
            if hasattr(model, "module"):
                ...
                model.eval()
                description = model.generate_description(test_embedding, tokenizer)
